    ...
    <div className="h-full flex flex-col bg-[#f0f4f8]">
       <div className="h-20 bg-white border-b border-blue-100 flex items-center justify-between px-8 shadow-sm">
         <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-black uppercase tracking-widest text-slate-800">ตรวจสอบคุณภาพ (QC)</h1>
         </div>
         <div className="px-4 py-2 bg-blue-500 text-white rounded-2xl text-xs font-black">
            {qcOrders.length} รายการรอตรวจสอบ
         </div>
      </div>
         ...
         <div className="w-1/3 bg-white rounded-[2rem] shadow-sm overflow-y-auto no-scrollbar border border-gray-100 p-4 space-y-3">
            {qcOrders.map((order: any) => (
              ...
              >
                 <div className="text-left">
                    ...
                    <h4 className="font-black text-lg text-slate-800 italic">{order.customerName || 'ลูกค้า Walk-in'}</h4>
                    <p className="text-[10px] font-bold text-gray-400">{order.items?.length} รายการ • {order.platform}</p>
                 </div>
            ...
            {qcOrders.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center opacity-20">
                 ...
                 <p className="mt-4 text-xs font-black uppercase tracking-widest">ตรวจสอบครบแล้ว</p>
              </div>
            )}
         </div>
              ...
              {selectedOrder ? (
                 ...
                 >
                    <div className="p-8 border-b border-gray-50">
                       <h2 className="text-2xl font-black italic mb-1 text-slate-800">ตรวจสอบ: {selectedOrder.customerName || 'ลูกค้า Walk-in'}</h2>
                       <p className="text-sm font-bold text-gray-400">เลขออเดอร์: {selectedOrder.orderNo}</p>
                    </div>

                    <div className="flex-1 p-8 overflow-y-auto space-y-8">
                       {/* Menu List */}
                       <div className="space-y-3">
                          <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 block">ตรวจสอบรายการเมนู</label>
                          {selectedOrder.items?.map((item: any, idx: number) => (
                             <div key={idx} className="flex gap-4 items-center p-4 bg-gray-50 rounded-2xl border border-gray-100">
                                <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center font-black text-slate-400">×{item.quantity}</div>
                                <div>
                       ...
                       <div className="space-y-3 pt-6 border-t border-gray-100">
                          <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 block">รายการตรวจสอบ (QC Checklist)</label>
                          {[
                            { id: 'menuCorrect', label: 'รายการและจำนวนถูกต้อง', sub: 'ตรวจสอบรายการและจำนวนตาม Ticket' },
                            { id: 'lidSecured', label: 'ปิดฝาและบรรจุภัณฑ์เรียบร้อย', sub: 'ปิดฝาแน่นสนิท ไม่มีการหกเลอะเทอะ' },
                            { id: 'stickerApplied', label: 'ติดสติกเกอร์ครบถ้วน', sub: 'ติดสติกเกอร์ชื่อลูกค้าและเมนูเรียบร้อย' }
                          ].map(item => (
                            <button 
                              key={item.id}
                    ...
                    <div className="p-8 bg-gray-50/50 border-t border-gray-100">
                       ...
                       >
                          ผ่านการตรวจสอบ (ส่งไปจุดรับสินค้า)
                       </button>
                    </div>
                 </motion.div>
              ) : (
                <div className="h-full flex flex-col items-center justify-center text-gray-300">
                   ...
                   <p className="mt-4 font-black text-xs uppercase tracking-widest">เลือกออเดอร์เพื่อเริ่มการตรวจสอบ</p>
                </div>
              )}
            </AnimatePresence>
         </div>
