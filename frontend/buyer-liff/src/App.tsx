  ...
  return (
    <div className="flex-1 overflow-y-auto no-scrollbar" style={{background:C.bg}}>
      ...
      <div className="px-5 pt-6 pb-4" style={{background:`linear-gradient(135deg, ${C.accent}, ${C.mid})`}}>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-lg overflow-hidden border-2 border-white/30">
               <img src="/logo01.png" alt="456 Coffee" className="w-10 h-10 object-contain" onError={(e)=>{e.currentTarget.style.display='none'}}/>
               <Coffee size={24} className="text-[#ffba0a] absolute" style={{zIndex:-1}}/>
            </div>
            <div>
              <p className="text-white/80 text-[10px] font-bold uppercase tracking-widest">456 Coffee</p>
              <h1 className="text-white font-black text-lg leading-tight">{user?.displayName||'สวัสดีครับ'}</h1>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button className="w-10 h-10 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center border border-white/10 hover:bg-white/30 transition-all"><Bell size={20} className="text-white"/></button>
            {user?.pictureUrl?<img src={user.pictureUrl} alt="" className="w-10 h-10 rounded-2xl border-2 border-white/50 shadow-md"/>:<div className="w-10 h-10 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center border border-white/10"><User size={20} className="text-white"/></div>}
          </div>
        </div>
        {/* Search bar */}
        <div className="relative">
          <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2" style={{color:C.muted}}/>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="ค้นหาเมนูโปรดของคุณ..." className="w-full bg-white/95 backdrop-blur-md rounded-2xl py-3.5 pl-12 pr-4 text-sm focus:outline-none shadow-xl transition-all focus:ring-2 focus:ring-white/50" style={{color:C.dark}}/>
        </div>
      </div>
      ...
      {loading?<div className="flex justify-center py-16"><Loader2 size={28} className="animate-spin" style={{color:C.accent}}/></div>:(
        <div className="space-y-6 pb-24">
          {/* Banner */}
          <div className="mx-5 mt-5 rounded-3xl overflow-hidden h-36 relative shadow-lg" style={{background:`linear-gradient(135deg, #5c4428, #5a3b16)`}}>
            <div className="absolute inset-0 p-5 flex flex-col justify-end">
              <span className="text-white/80 text-xs font-bold tracking-wider mb-1">โปรโมชั่น</span>
          ...
          <div>
            <div className="flex items-center justify-between px-5 mb-3">
              <h2 className="font-black text-[#252627] text-base flex items-center gap-2">เมนูแนะนำ <Sparkles size={16} className="text-amber-500 animate-pulse"/></h2>
              <button onClick={()=>onNavigate('menu')} className="text-xs font-bold text-[#ffba0a]">ดูทั้งหมด</button>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between px-5 mb-3">
              <h2 className="font-black text-[#252627] text-base">ร้านใกล้ฉัน 📍</h2>
            </div>
            <div className="space-y-3 px-5">
              {branches.filter(b=>b.isOpen).map(b=>(
                <motion.div key={b.id} whileTap={{scale:0.98}} onClick={()=>onSelectBranch(b)} className="bg-white rounded-2xl p-4 border border-[#ffedc4] flex items-center gap-3 shadow-sm active:shadow-inner transition-all">
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 bg-amber-50 text-[#ffba0a] shadow-inner"><Coffee size={22}/></div>
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-sm truncate text-[#252627]">{b.name}</p>
                    <div className="flex items-center gap-1.5 mt-0.5 text-[10px] text-gray-400 font-medium">
                      <MapPin size={10}/> <span className="truncate">{b.address || 'รายละเอียดสถานที่'}</span>
                    </div>
                    <div className="flex items-center gap-2 mt-2">
                      <span className={`text-[9px] px-2 py-0.5 rounded-full font-black ${b.isOpen?'bg-emerald-50 text-emerald-600 border border-emerald-100':'bg-gray-100 text-gray-500'}`}>{b.isOpen?'เปิด':'ปิด'}</span>
                      {b.openTime&&<span className="text-[9px] font-bold text-gray-400">{b.openTime}–{b.closeTime}</span>}
                    </div>
                  </div>
                  <ChevronRight size={18} className="text-[#ffedc4]"/>
                </motion.div>
              ))}
            </div>
          </div>
...
function MenuPage({onAddToCart}:{onAddToCart:(item:MenuItem,qty:number,opts:Record<string,MenuOption[]>,note:string)=>void;cart:CartItem[]}) {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden" style={{background:C.bg}}>
      <div className="px-4 pt-5 pb-3 bg-white border-b border-[#ffedc4] shadow-sm">
        <h1 className="font-black text-xl text-[#252627] mb-4 flex items-center gap-2 italic">เมนูของเรา ☕</h1>
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
          <motion.button whileTap={{scale:0.95}} onClick={()=>setActiveCat(null)} className={`shrink-0 px-5 py-2 rounded-full text-xs font-bold border transition-all ${!activeCat?'bg-[#ffba0a] text-white border-transparent shadow-md shadow-amber-900/20':'bg-white border-[#ffedc4] text-[#5a3b16]'}`}>ทั้งหมด</motion.button>
          {categories.map(cat=>(
            <motion.button key={cat.id} whileTap={{scale:0.95}} onClick={()=>setActiveCat(cat.id)} className={`shrink-0 px-5 py-2 rounded-full text-xs font-bold border transition-all ${activeCat===cat.id?'bg-[#ffba0a] text-white border-transparent shadow-md shadow-amber-900/20':'bg-white border-[#ffedc4] text-[#5a3b16]'}`}>
              {cat.icon} {cat.name}
            </motion.button>
          ))}
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 no-scrollbar">
        {loading?<div className="flex justify-center py-20"><Loader2 size={32} className="animate-spin text-[#ffba0a]"/></div>
        :filtered.length===0?<div className="flex flex-col items-center justify-center py-20 opacity-30"><Coffee size={64} className="mb-4 text-[#ffba0a]"/><p className="font-black">ไม่พบเมนูที่ค้นหา</p></div>
  ...
  if(cart.length===0)return(
    <div className="flex-1 flex flex-col items-center justify-center -mt-20" style={{background:C.bg}}>
      <div className="w-24 h-24 rounded-[2rem] bg-amber-50 flex items-center justify-center mb-6">
         <ShoppingBag size={48} className="text-[#ffba0a] opacity-40"/>
      </div>
      <p className="font-black text-xl text-[#252627]">ตะกร้าว่างอยู่</p>
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden" style={{background:C.bg}}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar">
        <h2 className="px-1 text-[10px] font-bold text-[#ffba0a] uppercase tracking-widest mb-1">รายการสินค้าของคุณ</h2>
        {cart.map((item,idx)=>(
          <motion.div key={idx} layout className="bg-white rounded-[2rem] p-4 border border-[#ffedc4] shadow-sm relative overflow-hidden">
            <div className="flex gap-4">
              <div className="w-20 h-20 rounded-2xl overflow-hidden shrink-0 bg-amber-50">
                {item.menuItem.imageUrl?<img src={item.menuItem.imageUrl} alt="" className="w-full h-full object-cover"/>:<Coffee size={24} className="m-auto mt-7 text-[#ffba0a]"/>}
              </div>
              <div className="flex-1 min-w-0 py-1">
                <div className="flex items-start justify-between">
                  <h3 className="font-bold text-[#252627] truncate text-sm">{item.menuItem.name}</h3>
                ...
                <div className="flex items-center justify-between mt-3">
                  <span className="font-black text-sm text-[#ffba0a]">฿{item.unitTotal}</span>
                  <div className="flex items-center gap-3 bg-gray-50 rounded-xl p-1 px-3 border border-gray-100 shadow-inner">
                    <button onClick={()=>onUpdateQty(idx,Math.max(1,item.quantity-1))} className="text-gray-400 hover:text-[#ffba0a] p-1"><Minus size={12} strokeWidth={3}/></button>
                    <span className="font-black text-xs w-4 text-center">{item.quantity}</span>
                    <button onClick={()=>onUpdateQty(idx,item.quantity+1)} className="text-gray-400 hover:text-[#ffba0a] p-1"><Plus size={12} strokeWidth={3}/></button>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="p-6 pb-8 bg-white border-t rounded-t-[3rem] shadow-2xl space-y-4">
        <button onClick={()=>setShowBranches(true)} className="flex items-center justify-between w-full p-4 rounded-2xl border-2 border-amber-50 bg-amber-50/20 group hover:border-[#ffba0a]/30 transition-all">
          <div className="flex items-center gap-3">
             <div className="w-10 h-10 rounded-xl bg-white border border-[#ffedc4] flex items-center justify-center text-[#ffba0a] group-hover:scale-105 transition-transform"><StoreIcon size={20}/></div>
             <div className="text-left"><p className="text-[10px] font-bold text-[#ffba0a] uppercase">สาขาที่รับ</p><p className="font-bold text-sm text-[#252627]">{selectedBranch?.name || 'เลือกสาขา...'}</p></div>
          </div>
          <ChevronRight size={18} className="text-[#ffba0a]"/>
        </button>
      ...
      <AnimatePresence>
        {showCheckout && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-50 flex items-end justify-center">
            ...
            <motion.div initial={{y:'100%'}} animate={{y:0}} exit={{y:'100%'}} transition={{type:'spring',damping:25,stiffness:200}} className="relative bg-[#f5f5f5] w-full rounded-t-[3rem] p-8 pb-10 max-h-[90vh] overflow-y-auto no-scrollbar shadow-2xl">
              <div className="w-12 h-1.5 bg-gray-200 rounded-full mx-auto mb-8" />
              
              <div className="space-y-8">
                <section>
                  <label className="text-[10px] font-bold text-[#5a3b16] uppercase tracking-wider mb-4 block mx-1">การรับสินค้า</label>
                  <div className="space-y-3">
                    <button onClick={()=>setFulfillType('PICKUP')} className={`w-full p-4 rounded-2xl border-2 flex items-center justify-between transition-all ${fulfillType==='PICKUP'?'border-[#ffba0a] bg-amber-50':'border-[#ffedc4] bg-white'}`}>
                      <div className="flex items-center gap-3"><Clock className={fulfillType==='PICKUP'?'text-[#ffba0a]':'text-gray-400'}/><div className="text-left"><p className="font-bold text-sm text-[#252627]">รับที่สาขา</p><p className="text-[10px] text-gray-500">รับสินค้าที่ร้าน</p></div></div>
                      {fulfillType==='PICKUP'&&<CheckCircle2 size={20} className="text-[#ffba0a]"/>}
                    </button>
                    <button onClick={()=>setFulfillType('DELIVERY')} className={`w-full p-4 rounded-2xl border-2 flex items-center justify-between transition-all ${fulfillType==='DELIVERY'?'border-[#ffba0a] bg-amber-50':'border-[#ffedc4] bg-white'}`}>
                      <div className="flex items-center gap-3"><Truck className={fulfillType==='DELIVERY'?'text-[#ffba0a]':'text-gray-400'}/><div className="text-left"><p className="font-bold text-sm text-[#252627]">จัดส่งถึงที่</p><p className="text-[10px] text-gray-500">จัดส่งให้คุณ (+฿20)</p></div></div>
                      {fulfillType==='DELIVERY'&&<CheckCircle2 size={20} className="text-[#ffba0a]"/>}
                    </button>
                  </div>

                  {fulfillType==='DELIVERY'&&(
                    <div className="mt-4 animate-in fade-in slide-in-from-top-2 duration-300">
                       <label className="text-[10px] font-bold text-[#5a3b16] uppercase tracking-wider mb-2 block mx-1">ที่อยู่จัดส่ง</label>
                       {selectedAddress ? (
                          <div className="p-4 rounded-2xl bg-white border border-[#ffedc4] flex items-center justify-between group shadow-sm">
                             <div className="flex items-center gap-3">
                                <div className="p-2.5 rounded-xl bg-amber-50 text-[#ffba0a]">{selectedAddress.label==='Home'?<Home size={18}/>:selectedAddress.label==='Office'?<Briefcase size={18}/>:<MapPin size={18}/>}</div>
                                <div><p className="font-bold text-sm text-[#252627]">{selectedAddress.label}</p><p className="text-[10px] text-gray-500 truncate w-48">{selectedAddress.address}</p></div>
                             </div>
                             <button onClick={()=>setShowAddressPicker(true)} className="px-3 py-1 rounded-lg bg-amber-50 text-[10px] font-bold text-[#ffba0a] uppercase">เปลี่ยน</button>
                          </div>
                       ) : (
                          <button onClick={()=>setShowAddressPicker(true)} className="w-full p-4 rounded-2xl border-2 border-dashed border-[#ffedc4] bg-white/50 text-sm font-bold text-[#5a3b16] flex items-center justify-center gap-2">
                ...
                <section>
                  <label className="text-[10px] font-bold text-[#5a3b16] uppercase tracking-wider mb-4 block mx-1">วิธีการชำระเงิน</label>
                  <div className="grid grid-cols-2 gap-3">
                    <button onClick={()=>setPayMethod('QR')} className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all shadow-sm ${payMethod==='QR'?'border-[#ffba0a] bg-amber-50 text-[#ffba0a]':'border-[#ffedc4] bg-white text-gray-400'}`}>
                      <QrCode size={28}/><span className="text-xs font-black">สแกนคิวอาร์ (QR Scan)</span>
                    </button>
                    <button onClick={()=>setPayMethod('CASH')} className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all shadow-sm ${payMethod==='CASH'?'border-[#ffba0a] bg-amber-50 text-[#ffba0a]':'border-[#ffedc4] bg-white text-gray-400'}`}>
                      <Banknote size={28}/><span className="text-xs font-black">เงินสด/หน้าร้าน (Cash)</span>
                    </button>
                  </div>
                </section>

                <section>
                  <label className="text-[10px] font-bold text-[#5a3b16] uppercase tracking-wider mb-2 block mx-1">หมายเหตุ (Optional)</label>
                  <textarea value={note} onChange={e=>setNote(e.target.value)} placeholder="เช่น ไม่ใส่น้ำตาล, แพ้ถั่ว..." className="w-full p-4 rounded-2xl border-2 border-[#ffedc4] bg-white text-sm focus:border-[#ffba0a] outline-none transition-all resize-none h-24 no-scrollbar shadow-inner" />
                </section>

                <div className="space-y-4 pt-6 border-t border-[#ffedc4]">
      ...
      <AnimatePresence>
        {showBranches && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            <div onClick={()=>setShowBranches(false)} className="absolute inset-0 bg-[#252627]/60 backdrop-blur-sm" />
            <motion.div initial={{scale:0.9,opacity:0}} animate={{scale:1,opacity:1}} exit={{scale:0.9,opacity:0}} className="relative bg-[#f5f5f5] w-full max-w-sm rounded-[3rem] p-8 max-h-[80vh] flex flex-col shadow-2xl">
              <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-black italic text-[#ffba0a]">เลือกสาขา</h3><button onClick={()=>setShowBranches(false)} className="p-2 border rounded-full bg-white"><X size={20}/></button></div>
              <div className="flex-1 overflow-y-auto space-y-3 no-scrollbar pb-4">
                {branches.map(b=>(
                  <button key={b.id} onClick={()=>{setSelectedBranch(b);setShowBranches(false);}} className={`w-full p-4 rounded-2xl border-2 text-left transition-all ${selectedBranch?.id===b.id?'border-[#ffba0a] bg-amber-50 shadow-sm':'border-[#ffedc4] bg-white group hover:border-[#ffba0a]/30'}`}>
                    <p className="font-black text-sm text-[#252627]">{b.name}</p>
         ...
         {showAddressPicker && (
            <div className="fixed inset-0 z-[120] flex items-end justify-center">
               ...
               <motion.div initial={{y:'100%'}} animate={{y:0}} exit={{y:'100%'}} transition={{type:'spring',damping:25,stiffness:200}} className="relative bg-white w-full rounded-t-[3rem] p-8 pb-10 max-h-[70vh] overflow-y-auto no-scrollbar shadow-2xl">
                  <div className="w-12 h-1.5 bg-gray-200 rounded-full mx-auto mb-8" />
                  <h3 className="text-2xl font-black mb-6 text-[#252627]">ที่อยู่จัดส่งของคุณ</h3>
                  <div className="space-y-3">
                     {member?.addresses?.map((addr:any)=>(
                        <button key={addr.id} onClick={()=>{setSelectedAddress(addr); setShowAddressPicker(false);}} className={`w-full p-5 rounded-3xl border-2 flex items-center gap-5 text-left transition-all ${selectedAddress?.id===addr.id?'border-[#ffba0a] bg-amber-50 shadow-sm':'border-[#ffedc4] bg-white'}`}>
                           <div className={`p-3 rounded-2xl shadow-sm ${selectedAddress?.id===addr.id?'bg-[#ffba0a] text-white':'bg-amber-50 text-[#ffba0a]'}`}>
                              {addr.label==='Home'?<Home size={22}/>:addr.label==='Office'?<Briefcase size={22}/>:<MapPin size={22}/>}
                           </div>
                           <div className="flex-1 min-w-0">
                              <p className="font-black text-sm text-[#252627]">{addr.label}</p>
                              <p className="text-xs text-gray-400 truncate mt-0.5">{addr.address}</p>
                           </div>
                           {selectedAddress?.id===addr.id && <CheckCircle2 size={24} className="text-[#ffba0a]"/>}
                        </button>
                     ))}
                     <button onClick={()=>setIsAddressFormOpen(true)} className="w-full p-5 rounded-3xl border-2 border-dashed border-[#ffedc4] text-sm font-black text-[#ffba0a] flex items-center justify-center gap-3 bg-amber-50/20 active:scale-95 transition-all">
  ...
  if(selected)return(
    <div className="flex-1 flex flex-col overflow-hidden" style={{background:C.bg}}>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4 no-scrollbar pb-24">
        {/* Status */}
        <div className="bg-white rounded-[2rem] p-6 border border-[#ffedc4] shadow-sm">
          <div className="flex items-center justify-between mb-5">
            <div>
               <p className="text-[10px] font-bold text-gray-400 tracking-widest uppercase mb-1">{selected.orderNo}</p>
               <div className="flex items-center gap-3">
                  <span className="text-5xl font-black italic text-[#ffba0a]">#{selected.queueNo || '---'}</span>
                  <div className="h-10 w-px bg-gray-100 mx-1"/>
                  <span className="font-black text-xl text-[#252627]">฿{selected.totalAmount}</span>
               </div>
            </div>
          ...
          {selected.status!=='CANCELLED'&&(
            <div className="flex gap-2.5 mt-2">
              {['PENDING','PAID','PREPARING','READY','COMPLETED'].map((s,i)=>{
                ...
                return <div key={s} className="flex-1"><div className={`h-2 rounded-full transition-all duration-500 shadow-inner ${isActive?'bg-[#ffba0a] scale-y-110':'bg-gray-100'}`}/></div>;
              })}
            </div>
          )}
          {(selected.status==='READY')&&<div className="mt-5 p-4 rounded-2xl bg-emerald-50 border border-emerald-100 flex items-center justify-center gap-3 animate-pulse"><CheckCircle2 className="text-emerald-600" size={20}/><p className="text-xs font-black text-emerald-700 uppercase tracking-wider">กาแฟของคุณพร้อมแล้ว!</p></div>}
        </div>
        {/* Items */}
        <div className="bg-white rounded-[2rem] p-6 border border-[#ffedc4] shadow-sm">
          <p className="font-black text-sm text-[#252627] mb-4 uppercase tracking-wider border-b border-gray-50 pb-2">สรุปรายการสั่งซื้อ ({selected.items?.length||0})</p>
          <div className="space-y-4">{selected.items?.map((it:any,i:number)=>(
            <div key={i} className="flex justify-between items-start text-sm group">
              <div className="flex-1">
                <p className="font-bold text-[#252627]">{it.productName||it.product?.name} <span className="text-[#ffba0a]">×{it.quantity}</span></p>
                {it.selectedOptions && <p className="text-[10px] text-gray-400 mt-1">{JSON.parse(it.selectedOptions || '[]').map((o:any)=>o.label).join(', ')}</p>}
              </div>
              <span className="font-black text-[#ffba0a] ml-4">฿{it.price}</span>
            </div>
          ))}</div>
          <div className="mt-6 pt-4 border-t border-dashed border-gray-100 flex justify-between items-center text-[#252627] font-black text-lg">
             <span>ยอดรวม</span>
  ...
  return(
    <div className="flex-1 flex flex-col overflow-hidden" style={{background:C.bg}}>
      ...
      <div className="flex-1 overflow-y-auto p-4 no-scrollbar pb-32">
        {orders.length===0?(
          <div className="flex flex-col items-center justify-center py-24 opacity-30"><ClipboardList size={64} className="mb-4 text-[#ffba0a]"/><p className="font-black text-lg">ยังไม่มีคำสั่งซื้อ</p></div>
        ):(
          <div className="space-y-4">{orders.map(o=>{
            const cfg=STATUS_CFG[o.status];
            return(
              <motion.div key={o.id} whileTap={{scale:0.98}} onClick={()=>setSelected(o)} className="bg-white rounded-[2rem] p-5 border border-[#ffedc4] shadow-sm active:shadow-inner transition-all">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-3xl font-black italic text-[#ffba0a]">#{o.queueNo || '---'}</span>
                ...
                <p className="font-bold text-sm mb-2 text-[#252627] truncate">{o.items?.slice(0,2).map((i:any)=>i.productName||i.product?.name).join(', ')}{(o.items?.length||0)>2?` +${o.items.length-2} more`:''}</p>
                <div className="flex justify-between items-center border-t border-gray-50 pt-3">
                  ...
                  <span className="font-black text-base text-[#252627]">฿{o.totalAmount}</span>
                </div>
              </motion.div>
            );
          })}</div>
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[200] flex items-end justify-center">
          ...
          <motion.div initial={{y:'100%'}} animate={{y:0}} exit={{y:'100%'}} transition={{type:'spring',damping:25,stiffness:200}} className="relative bg-[#f5f5f5] w-full rounded-t-[3rem] p-8 pb-12 shadow-2xl border-t-4 border-white">
            <div className="w-12 h-1.5 bg-gray-200 rounded-full mx-auto mb-8" />
            <h3 className="text-2xl font-black mb-6 text-[#252627] italic uppercase tracking-tight">{initialData ? 'แก้ไขที่อยู่' : 'เพิ่มที่อยู่ใหม่'}</h3>
            
            <div className="space-y-6">
               <section>
                  <label className="text-[10px] font-bold text-[#ffba0a] uppercase tracking-widest mb-3 block ml-1">ประเภทที่อยู่</label>
                  <div className="flex gap-2">
                     {['Home', 'Office', 'Other'].map(l => (
                        <button key={l} onClick={()=>setLabel(l)} className={`flex-1 py-3 rounded-2xl font-black text-xs transition-all border-2 ${label===l ? 'bg-[#ffba0a] text-white border-transparent shadow-lg' : 'bg-white text-gray-400 border-gray-100'}`}>{l}</button>
                     ))}
                  </div>
               </section>

               <section>
                  <label className="text-[10px] font-bold text-[#ffba0a] uppercase tracking-widest mb-3 block ml-1">รายละเอียดที่อยู่</label>
        ...
        <div className="fixed inset-0 z-[180] flex items-end justify-center">
          ...
          <motion.div initial={{y:'100%'}} animate={{y:0}} exit={{y:'100%'}} transition={{type:'spring',damping:25,stiffness:200}} className="relative bg-white w-full rounded-t-[3rem] max-h-[85vh] overflow-hidden flex flex-col shadow-2xl">
            <div className="p-8 pb-4 flex justify-between items-center shrink-0">
               <h3 className="text-2xl font-black text-[#252627] italic uppercase tracking-tight">ที่อยู่ที่บันทึกไว้</h3>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 no-scrollbar pb-32">
               {member?.addresses?.map((addr: any) => (
                  <div key={addr.id} className="bg-white p-5 rounded-3xl border-2 border-gray-50 shadow-sm flex items-center gap-4 group">
                     <div className="w-12 h-12 rounded-2xl bg-amber-50 text-[#ffba0a] flex items-center justify-center shrink-0 shadow-inner">
                        {addr.label==='Home' ? <Home size={22}/> : addr.label==='Office' ? <Briefcase size={22}/> : <MapPin size={22}/>}
                     </div>
                     <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                           <p className="font-black text-sm text-[#252627] uppercase">{addr.label}</p>
                           {addr.isDefault && <span className="text-[8px] bg-[#ffba0a] text-white px-2 py-0.5 rounded-full font-black italic">DEFAULT</span>}
                        </div>
                        <p className="text-xs text-gray-400 truncate">{addr.address}</p>
                     </div>
                     <div className="flex items-center gap-1">
                        <button onClick={()=>onEdit(addr)} className="p-2 text-gray-300 hover:text-[#ffba0a] transition-colors"><Settings size={18}/></button>
               ...
               {(!member?.addresses || member.addresses.length === 0) && (
                  <div className="text-center py-20 opacity-20">
                     ...
                     <p className="font-black">คุณยังไม่มีที่อยู่ที่บันทึกไว้</p>
                  </div>
               )}
            </div>
    ...
    <div className="flex-1 overflow-y-auto no-scrollbar" style={{background:C.bg}}>
      <div className="px-5 pt-8 pb-6 shadow-sm" style={{background:`linear-gradient(135deg, ${C.accent}, ${C.mid})`}}>
        ...
        <div className="mt-6 bg-white/10 backdrop-blur-md rounded-3xl p-5 flex items-center justify-between border border-white/20 shadow-xl">
          <div>
            ...
            <p className="text-white font-black text-3xl flex items-baseline gap-1">{member?.points||0} <span className="text-sm font-bold opacity-70">แต้ม</span></p>
          </div>
          <motion.button whileTap={{scale:0.95}} className="bg-white text-[#ffba0a] px-6 py-2.5 rounded-2xl text-[10px] font-black shadow-xl uppercase tracking-wider">แลกแต้ม</motion.button>
        </div>
      </div>
      
      {/* Saved Addresses Summary */}
      <div className="px-4 mt-6">
         <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-[#ffedc4]">
            <div className="flex items-center justify-between mb-4">
               <h3 className="font-black text-[11px] text-[#252627] uppercase tracking-widest flex items-center gap-2">ที่อยู่ของฉัน</h3>
               <button onClick={()=>setShowManage(true)} className="text-[10px] font-bold text-[#ffba0a] underline">จัดการ</button>
            </div>
            <div className="space-y-4">
               {member?.addresses?.length > 0 ? member.addresses.map((addr:any)=>(
                  <div key={addr.id} className="flex items-start gap-3">
                     <div className="bg-amber-50 p-1.5 rounded-lg text-[#ffba0a]">{addr.label==='Home'?<Home size={12}/> : addr.label==='Office'?<Briefcase size={12}/> : <MapPin size={12}/>}</div>
                     <div className="flex-1 min-w-0">
                        <p className="text-[10px] font-black text-[#252627] uppercase mb-0.5">{addr.label}</p>
                        <p className="text-xs text-gray-400 truncate">{addr.address}</p>
                     </div>
                  </div>
      ...
      <div className="p-4 pb-32 space-y-2.5 mt-2">
        {menus.map((m,i)=>{const Icon=m.icon;return(
          <motion.button key={i} whileTap={{scale:0.98}} onClick={m.action} className="w-full bg-white rounded-2xl p-4 border border-[#ffedc4] flex items-center gap-4 shadow-sm text-left group transition-all">
            <div className="w-11 h-11 rounded-xl flex items-center justify-center bg-gray-50 text-[#ffba0a] group-active:bg-amber-50 transition-colors shadow-inner"><Icon size={20}/></div>
            <div className="flex-1">
               <p className="font-bold text-sm text-[#252627]">{m.label}</p>
               {m.sub&&<p className="text-[10px] font-bold text-[#ffba0a] uppercase tracking-wider">{m.sub}</p>}
            </div>
            <ChevronRight size={18} className="text-[#ffedc4]"/>
          </motion.button>
        );})}
        <div className="pt-4 px-2">
          <motion.button whileTap={{scale:0.95}} className="w-full bg-rose-50 border border-rose-100 rounded-2xl p-4 flex items-center gap-4 shadow-sm">
            ...
            <span className="font-black text-sm text-rose-600 uppercase tracking-wider">ออกจากระบบ</span>
          </motion.button>
        </div>
      </div>
      ...
      <AnimatePresence>
         {internalShowHistory && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
               ...
               <motion.div initial={{scale:0.9, opacity:0}} animate={{scale:1, opacity:1}} exit={{scale:0.9, opacity:0}} className="relative bg-[#f5f5f5] w-full max-w-sm rounded-[3rem] overflow-hidden max-h-[80vh] flex flex-col shadow-2xl border-4 border-white">
                  <div className="p-6 border-b border-[#ffedc4] flex items-center justify-between shrink-0 bg-white">
                     <h3 className="font-black text-lg italic text-[#ffba0a]">ประวัติแต้ม</h3>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar pb-8">
                     {member?.pointHistory?.map((h:any)=>(
                        <motion.div initial={{opacity:0, x:-20}} animate={{opacity:1, x:0}} key={h.id} className="bg-white p-4 rounded-2xl border border-[#ffedc4] flex items-center justify-between shadow-sm">
                           <div className="flex items-center gap-3">
                              ...
                              <div>
                                 <p className="text-xs font-black text-[#252627] uppercase">{h.reason}</p>
                     ...
                     {(!member?.pointHistory || member.pointHistory.length === 0) && (
                        <div className="text-center py-24 opacity-20">
                           ...
                           <p className="font-black">ยังไม่มีข้อมูล</p>
                        </div>
                     )}
                  </div>
               </motion.div>
...
export default function App() {
  ...
  if(isLiffLoading)return(
    <div className="min-h-screen flex flex-col items-center justify-center" style={{background:C.bg}}>
      <Coffee size={48} className="mb-4 text-[#ffba0a]"/><Loader2 size={24} className="animate-spin text-[#ffba0a]"/>
      <p className="mt-4 font-black uppercase tracking-widest text-[#ffba0a] text-xs">กำลังโหลด...</p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex flex-col max-w-md mx-auto relative overflow-hidden shadow-2xl" style={{background:C.bg}}>
      ...
      <AnimatePresence>{justOrdered&&(
        <motion.div initial={{opacity:0,y:-50, scale:0.8}} animate={{opacity:1,y:0, scale:1}} exit={{opacity:0,y:-50}} className="fixed top-6 left-4 right-4 z-[150] bg-emerald-600 text-white p-5 rounded-3xl shadow-2xl flex items-center justify-between border-2 border-white/20">
          <div className="flex items-center gap-4">
             ...
             <div><p className="font-black text-sm uppercase tracking-wider">สั่งซื้อสำเร็จ!</p><p className="text-[10px] font-bold opacity-80">เรากำลังเตรียมกาแฟให้คุณ...</p></div>
          </div>
          <div className="bg-black/20 px-4 py-2 rounded-2xl text-[10px] font-black italic">แต้ม +{Math.floor(cart.reduce((s,i)=>s+i.unitTotal,0)/10)}</div>
        </motion.div>
      )}</AnimatePresence>
      ...
      {/* Bottom navigation */}
      <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-white/80 backdrop-blur-md border-t border-[#ffedc4] z-40 pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.03)]">
          ...
          {TABS.map(t=>{
            ...
            return(
              <button key={t.id} onClick={()=>setTab(t.id)} className="flex-1 flex flex-col items-center justify-center relative transition-all active:scale-90">
                <div className={`p-2 rounded-2xl transition-all duration-300 ${isActive?'bg-amber-50 text-[#ffba0a] scale-110 shadow-inner':'text-gray-300'}`}>
                ...
                <span className={`text-[9px] mt-1 font-black uppercase tracking-widest transition-opacity duration-300 ${isActive?'opacity-100 text-[#ffba0a]':'opacity-0'}`}>{t.label}</span>
              </button>
            );
          })}
        </div>
